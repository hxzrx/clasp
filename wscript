...
def options(ctx):
    ...
    ctx.add_option(
        ...
        dest="wscript_config",
        action="store",
        default="wscript.config",
        help="wscript config file [default: wscript.config]",
    )
...
GCS_NAMES = ["boehm", "boehmprecise", "mmtk", "mmtkprecise", "preciseprep"]
...
def update_dependencies(cfg):
    ...
    fetch_git_revision(
        ...
        revision="dddb4d8af3eae78017baae7fb9b99e73d2a56e6b",
    )
    ...
    fetch_git_revision(
        "src/scraper/dependencies/alexandria",
        "https://github.com/clasp-developers/alexandria.git",
        "e5c54bc30b0887c237bde2827036d17315f88737",
    )
    fetch_git_revision(
        "src/scraper/dependencies/esrap",
        "https://github.com/scymtym/esrap.git",
        "c99c33a33ff58ca85e8ba73912eba45d458eaa72",
    )
    fetch_git_revision(
        "src/scraper/dependencies/trivial-with-current-source-form",
...
def analyze_clasp(cfg):
    ...
    if len(cfg.extensions_sif_nodes) > 2:
        raise Exception(
            "You can only run the static analyzer on clasp or clasp+one-extension - you have: %s"
            % cfg.extensions_sif_nodes
        )
    output_file = generate_output_filename(cfg.extensions_sif_nodes)
    run_program_echo(
        "build/boehm/iclasp-boehm",
        "-N",
        "-D",
        "--feature",
        "ignore-extensions",
        "--load",
        "sys:modules;clasp-analyzer;run-serial-analyzer.lisp",
        "--eval",
        '(run-search "%s")' % (output_file),
        "--eval",
        "(core:quit)",
    )
    print(
...
def analyze_test(cfg):
    ...
    run_program_echo(
        ...
        "sys:modules;clasp-analyzer;run-serial-analyzer.lisp",
        "--eval",
        '(run-search "%s" :selection-pattern "%s")' % (output_file, selection_pattern),
        "--eval",
        "(core:quit)",
    )
    print(
...
def test(cfg):
    ...
    run_program_echo(
        ...
        "ignore-extensions",
        "--load",
        "sys:regression-tests;run-all.lisp",
        "--eval",
        '(progn (format t "~%Test done~%")(core:quit))',
    )
...
def test1(cfg):
    log.debug("Execute regression tests\n")
    run_program_echo(
        "build/boehmprecise/iclasp-boehmprecise -t a",
        "--feature",
        "ignore-extensions",
        "--load",
        "sys:regression-tests;run-one.lisp",
        "--eval",
        '(progn (format t "~%Test done~%")(core:quit))',
    )
...
def configure_common(cfg, variant_obj):
    ...
    cfg.define(
        "BUILD_STLIB",
        libraries_as_link_flags_as_string(cfg.env.STLIB_ST, cfg.env.STLIB),
    )
    cfg.define(
        "BUILD_LIB", libraries_as_link_flags_as_string(cfg.env.LIB_ST, cfg.env.LIB)
    )
    log.debug("cfg.env.LINKFLAGS = %s", cfg.env.LINKFLAGS)
    log.debug("cfg.env.LDFLAGS = %s", cfg.env.LDFLAGS)
    cfg.define(
        "BUILD_LINKFLAGS", " ".join(cfg.env.LINKFLAGS) + " " + " ".join(cfg.env.LDFLAGS)
    )
...
class variant(object):
    ...
    def fasl_name(self, build, stage=None):
        ...
        if build.env.CLASP_BUILD_MODE == "fasl":
            return build.path.find_or_declare(
                "fasl/%s%s-%s%s%s-image.lfasl"
                % (
                    use_stage,
                    APP_NAME,
                    self.gc_name,
    ...
    def snapshot_node(self, build, appname=APP_NAME, stage=None):
        if stage == None:
            ...
            use_stage = stage
        return build.path.find_or_declare(
            "generated/%s%s-%s%s%s-snapshot.snapshot"
            % (
                use_stage,
                appname,
                self.gc_name,
    ...
    def snapshot_object_node(self, build, appname=APP_NAME, stage=None):
        if stage == None:
            ...
            use_stage = stage
        return build.path.find_or_declare(
            "generated/%s%s-%s%s%s-snapshot.o"
            % (
                use_stage,
                appname,
                self.gc_name,
    ...
    def common_lisp_output_name_list(
        self, build, input_files, stage=None, variant=None
    ):
        if stage == None:
            use_stage = self.stage_char
        else:
        ...
        nodes = waf_nodes_for_object_files(
            build, input_files, self.fasl_dir(stage=use_stage)
        )
        return nodes
    ...
    def variant_dir(self):
        return "%s%s%s" % (
            self.gc_name,
            self.mpi_dir_extension(),
            self.debug_dir_extension(),
        )
    ...
    def configure_for_release(self, cfg):
        ...
        if os.getenv("CLASP_RELEASE_CXXFLAGS") != None:
            cfg.env.append_value(
                "CXXFLAGS", os.getenv("CLASP_RELEASE_CXXFLAGS").split()
            )
        if os.getenv("CLASP_RELEASE_LINKFLAGS") != None:
            cfg.env.append_value(
                "LINKFLAGS", os.getenv("CLASP_RELEASE_LINKFLAGS").split()
            )
    ...
    def configure_for_debug(self, cfg):
        ...
        if os.getenv("CLASP_DEBUG_LINKFLAGS") != None:
            cfg.env.append_value(
                "LINKFLAGS", os.getenv("CLASP_DEBUG_LINKFLAGS").split()
            )
    ...
    def configure_variant(self, cfg, env_copy):
        ...
        if cfg.env["DEST_OS"] == DARWIN_OS:
            ...
            if cfg.env.LTO_FLAG:
                cfg.env.append_value(
                    "LDFLAGS",
                    "-Wl,-object_path_lto,%s_lib.lto.o" % self.executable_name(),
                )
        log.info(
            "Setting up boehm library cfg.env.STLIB_BOEHM = %s ", cfg.env.STLIB_BOEHM
        )
...
def configure(cfg):
    def update_exe_search_path(cfg):
        ...
        if cfg.env["DEST_OS"] == DARWIN_OS:
            ...
            if "-isysroot" in cfg.env["CPPFLAGS"]:
                if XCODE_SDK in cfg.env["CPPFLAGS"]:
                    ...
                    log.info(
                        "There is a mismatch in the -isysroot option in wscript.config - you provide %s and XCODE_SDK is %s"
                        % (cfg.env["CPPFLAGS"], XCODE_SDK)
                    )
                    exit(1)
            else:
            ...
            if len(llvm_config_binary) == 0:
                llvm_paths = glob.glob(
                    "/usr/local/opt/llvm@%s/bin/llvm-config" % LLVM_VERSION
                )
                if len(llvm_paths) >= 1:
            ...
            if len(llvm_config_binary) == 0:
                ...
                for candidate in [
                    ...
                    try:
                        ...
                        cfg.to_log(
                            candidate % LLVM_VERSION + " was not found (ignoring)"
                        )
                ...
                log.info(
                    "On %s looking for %s" % (cfg.env["DEST_OS"], llvm_config_binary)
                )
        ...
        assert os.path.isdir(
            externals_bin_dir
        ), "Please provide a valid LLVM_CONFIG_BINARY. See the 'wscript.config.template' file."

    def check_externals_clasp_version(cfg):
        log.debug(
            "check_externals_clasp_version begins, cfg.env.LLVM_CONFIG_BINARY = %s",
            cfg.env.LLVM_CONFIG_BINARY,
        )
        ...
        if not correct_version:
            raise Exception(
                "You do not have the correct version of externals-clasp installed - you need the one with the LLVM_COMMIT=%s"
                % externals_clasp_llvm_hash
            )

    def load_local_config(cfg):
        local_environment = {}
        if os.path.isfile(cfg.options.wscript_config):
            log.debug("local_environment = %s", local_environment)
            exec(open(cfg.options.wscript_config).read(), globals(), local_environment)
            for key in local_environment.keys():
                if not key in VALID_OPTIONS:
                    raise Exception(
                        "%s is an INVALID wscript.config option - valid options are: %s"
                        % (key, VALID_OPTIONS)
                    )
                else:
                    log.info(
                        "wscript.config option %s = %s", key, local_environment[key]
                    )
            cfg.env.update(local_environment)
        else:
            log.warn("wscript config file not found: %s", cfg.options.wscript_config)
    ...
    cfg.env["BUILD_ROOT"] = os.path.abspath(
        top
    ...
    if cfg.env.LLVM_CONFIG_BINARY_FOR_LIBS:
        log.warn(
            "Using a separate llvm-config binary for linking with the LLVM libs: %s",
            cfg.env.LLVM_CONFIG_BINARY_FOR_LIBS,
        )
    else:
        log.debug(
            "Using the same llvm-config binary for linking with the LLVM libs: %s",
            cfg.env.LLVM_CONFIG_BINARY,
        )
    ...
    if cfg.env["CLASP_BUILD_MODE"] == "bitcode":
        ...
        raise Exception(
            "CLASP_BUILD_MODE can only be 'thinlto'(default), 'lto', or 'object' - you provided %s"
            % cfg.env["CLASP_BUILD_MODE"]
        )
    log.info(
        "default cfg.env.CLASP_BUILD_MODE = %s, final cfg.env.LTO_FLAG = '%s'",
        cfg.env.CLASP_BUILD_MODE,
        cfg.env.LTO_FLAG,
    )
    ...
    log.debug(
        "cfg.env['USE_COMPILE_FILE_PARALLEL'] = %s",
        cfg.env["USE_COMPILE_FILE_PARALLEL"],
    )
    if (not "USE_COMPILE_FILE_PARALLEL" in cfg.env) or cfg.env[
        "USE_COMPILE_FILE_PARALLEL"
    ]:
    ...
    log.debug(
        "cfg.env['FORCE_STARTUP_EXTERNAL_LINKAGE'] = %s",
        cfg.env["FORCE_STARTUP_EXTERNAL_LINKAGE"],
    )
    if (not "FORCE_STARTUP_EXTERNAL_LINKAGE" in cfg.env) or cfg.env[
        "FORCE_STARTUP_EXTERNAL_LINKAGE"
    ]:
    ...
    if llvm_version_test and (int(cur_clang_version.split(".")[0]) != LLVM_VERSION):
        raise Exception(
            "You must have clang/llvm version %d installed - you have %s"
            % (LLVM_VERSION, cur_clang_version.split(".")[0])
        )
    # find a lisp for the scraper
    if not cfg.env.SCRAPER_LISP:
        cfg.env["SBCL"] = cfg.find_program("sbcl", var="SBCL")[0]
        cfg.env["SCRAPER_LISP"] = [
            cfg.env.SBCL,
            "--noinform",
            "--dynamic-space-size",
            "2048",
        ...
        if int(version_parts[0]) < SBCL_VERSION[0]:
            raise Exception(
                "The sbcl version is %s but it must be %s or higher"
                % (version, SBCL_VERSION_STRING)
            )
    ...
    for onelib in boost_libs:
        if cfg.env["LINK_STATIC"]:
            cfg.check_cxx(
                stlib=onelib, cflags="-Wall", uselib_store="BOOST-%s" % onelib
            )
        else:
    ...
    llvm_libraries = [
        x[2:] for x in run_llvm_config_for_libs(cfg, "--libs").split()
    ]  # drop the '-l' prefixes
    # dynamic llvm/clang
    cfg.check_cxx(
        lib=CLANG_LIBRARIES, cflags="-Wall", uselib_store="CLANG", libpath=llvm_lib_dir
    )
    cfg.check_cxx(
        lib=llvm_libraries, cflags="-Wall", uselib_store="LLVM", libpath=llvm_lib_dir
    )
    ...
    cfg.define(
        "USE_COMPILED_CLOSURE", 1
    )  # disable this in the future and switch to ClosureWithSlots
    ...
    log.info(
        "Build mode '%s' linker in use '%s'" % (cfg.env.CLASP_BUILD_MODE, linker_in_use)
    )
    ...
    if cfg.env.SANITIZE_MEMORY:
        cfg.env.append_value(
            "CXXFLAGS", ["-fsanitize=memory", "-fsanitize-memory-track-origins=1"]
        )
        cfg.env.append_value(
            "CFLAGS", ["-fsanitize=memory", "-fsanitize-memory-track-origins=1"]
        )
        cfg.env.append_value(
            "LINKFLAGS", ["-fsanitize=memory", "-fsanitize-memory-track-origins=1"]
        )
    ...
    if cfg.env.DEBUG_OPTIONS:
        ...
        for opt in cfg.env.DEBUG_OPTIONS:
            if opt in DEBUG_OPTIONS:
                ...
                raise Exception(
                    "Illegal DEBUG_OPTION %s - allowed options: %s"
                    % (opt, DEBUG_OPTIONS)
                )
    ...
    log.info(
        "About to setup variants - cfg.options.enabl_empi = %s" % cfg.options.enable_mpi
    )
    if cfg.options.enable_mpi:
    ...
    for gc in GCS_NAMES:
        for enable_mpi in mpi_variants:
            for debug_build in [True, False]:
                ...
                log.pprint(
                    "Blue", "Setting up variant: %s" % variant_instance.variant_dir()
                )
                log.pprint(
                    "BLUE",
                    "variant_name = %s    variant_instance = %s"
                    % (variant_name, variant_instance),
                )
...
def post_build_hook(bld):
    ...
    log.info(
        "Compilation finished in %s, at %s",
...
def build(bld):
    ...
    if not bld.variant:
        bld.fatal(
            "Call waf with build_variant, e.g. './waf --jobs 2 --verbose build_cboehm'"
        )
    ...
    log.reinitialize(
        console_level=logging.DEBUG if Logs.verbose >= 1 else logging.INFO,
        log_file=os.path.join(
            bld.path.abspath(), out, bld.variant_obj.variant_dir(), "build.log"
        ),
    )

    log.debug(
        "build() starts, CLASP_BUILD_MODE = %s, options: %s",
    ...
    bld.iclasp_executable = bld.path.find_or_declare(
        bld.variant_obj.executable_name(stage="i")
    )

    bld.cclasp_link_product = bld.variant_obj.fasl_name(bld, stage="c")
    bld.cclasp_asdf_fasl = bld.path.find_or_declare(
        module_fasl_extension(
            bld, "%s/src/lisp/modules/asdf/asdf" % bld.variant_obj.fasl_dir(stage="c")
        )
    )
    ...
    install(
        "lib/clasp/", collect_waf_nodes(bld, "src/lisp/")
    )  # , suffix = [".lsp", ".lisp", ".asd"]))
    ...
    if bld.variant in bld.all_envs:
        ...
        log.pprint(
            "RED",
            "The bld.variant %s is not configured - run ./waf configure --enable-mpi"
            % bld.variant,
        )
        exit(1)
    ...
    include_dirs.append(
        "%s/%s/%s/generated/" % (bld.path.abspath(), out, bld.variant_obj.variant_dir())
    )
    ...
    intrinsics_bitcode_node = bld.path.find_or_declare(
        bld.variant_obj.inline_bitcode_archive_name("intrinsics")
    )
    builtins_bitcode_node = bld.path.find_or_declare(
        bld.variant_obj.inline_bitcode_archive_name("builtins")
    )
    builtins_no_debug_info_bitcode_node = bld.path.find_or_declare(
        bld.variant_obj.inline_bitcode_archive_name("builtins-no-debug-info")
    )
    cxx_all_bitcode_node = bld.path.find_or_declare(
        bld.variant_obj.cxx_all_bitcode_name()
    )
    out_dir_node = bld.path.find_dir(out)
    bclasp_symlink_node = out_dir_node.make_node("bclasp")
    bld.clasp_taskgen = bld.program(
        source=clasp_c_source_files,
    ...
    if bld.stage_val <= -1:
        ...
        task.set_inputs(
            [bld.iclasp_executable, intrinsics_bitcode_node, builtins_bitcode_node]
            + waf_nodes_for_lisp_files(bld, bld.clasp_aclasp)
        )
        aclasp_common_lisp_output_name_list = (
            bld.variant_obj.common_lisp_output_name_list(
                bld, bld.clasp_aclasp, stage="a"
            )
        )
    ...
    if bld.stage_val >= 1:
        ...
        log.debug(
            "find_or_declare aclasp_common_lisp_output_name_list = %s",
            aclasp_common_lisp_output_name_list,
        )
        task.set_outputs(aclasp_common_lisp_output_name_list)
        bld.add_to_group(task)
        ...
        task.set_inputs(
            [bld.iclasp_executable, builtins_bitcode_node, intrinsics_bitcode_node]
            + aclasp_common_lisp_output_name_list
        )
    ...
    if bld.stage_val >= 2:
        ...
        bclasp_common_lisp_output_name_list = (
            bld.variant_obj.common_lisp_output_name_list(
                bld, bld.clasp_bclasp, stage="b"
            )
        )
        log.debug(
            "find_or_declare bclasp_common_lisp_output_name_list = %s",
            bclasp_common_lisp_output_name_list,
        )
    ...
    if bld.stage_val >= 3:
        ...
        if True:
            ...
            serve_event_fasl = bld.path.find_or_declare(
                module_fasl_extension(
                    bld,
                    "%s/src/lisp/modules/serve-event/serve-event"
                    % bld.variant_obj.fasl_dir(stage="c"),
                )
            )
            print("serve_event_fasl = %s\n" % serve_event_fasl.abspath())
            task = compile_module(env=bld.env)
            task.set_inputs(
                [bld.iclasp_executable, bld.cclasp_link_product]
                + waf_nodes_for_lisp_files(
                    bld, ["src/lisp/modules/serve-event/serve-event"]
                )
            )
            ...
            if generate_dwarf(bld):
                serve_event_dwarf_file = bld.path.find_or_declare(
                    "%s/src/lisp/modules/serve-event/serve-event.fasl.dwarf"
                    % bld.variant_obj.fasl_dir(stage="c")
                )
            ...
            if generate_dwarf(bld):
                cclasp_asdf_dwarf_file = bld.path.find_or_declare(
                    "%s/src/lisp/modules/asdf/asdf.fasl.dwarf"
                    % bld.variant_obj.fasl_dir(stage="c")
                )
    ...
    if bld.stage_val >= 4:
        ...
        bld_extensions.set_inputs(
            [bld.iclasp_executable, bld.cclasp_link_product, bld.cclasp_asdf_fasl]
            + bld.extensions_startup_load_output_nodes
        )
        ...
        embed_snapshot(
            bld, snapshot_file, bld.iclasp_executable, bld.dclasp_executable, "clasp"
        )
...
def make_run_dsymutil_task(bld, stage_char, clasp_lto_o):
    if bld.env["DEST_OS"] == DARWIN_OS:
        ...
        dsym_file = bld.path.find_or_declare(
            "%s.dSYM" % variant.executable_name(stage=stage_char)
        )
        dsym_nodes = dsym_waf_nodes(
            variant.executable_name(stage=stage_char), dsym_file
        )
...
class link_fasl(clasp_task):
    def run(self):
        if (
            self.env.CLASP_BUILD_MODE == "faso"
            ...
            if self.env.LTO_FLAG:
                ...
                if self.env.USE_PARALLEL_BUILD and self.bld.stage_val < 3:
                    ...
                    log.debug(
                        "With USE_PARALLEL_BUILD and stage_val = %d dropping down to -O0 for link_fasl",
                        self.bld.stage_val,
                    )
                else:
...
class run_aclasp(clasp_task):
    def run(self):
        ...
        cmd = self.clasp_command_line(
            executable,
            image=False,
            features=["no-implicit-compilation", "jit-log-symbols", "clasp-min"],
            forms=['(load "sys:kernel;clasp-builder.lsp")', "(load-aclasp)"],
...
class compile_cclasp(clasp_task):
    def run(self):
        ...
        output_file = self.outputs[0].abspath()
        log.debug(
            "In compile_cclasp %s --image %s -> %s", executable, image_file, output_file
        )
        forms = [
        ...
        if self.bld.options.LOAD_CCLASP:
            ...
            forms += [
                '(core:compile-cclasp :output-file #P"%s")' % output_file,
                "(core:exit)",
            ]
        cmd = self.clasp_command_line(
            executable,
...
class recompile_cclasp(clasp_task):
    def run(self):
        ...
        if not os.path.isfile(other_clasp):
            raise Exception(
                "To use the recompile targets you need to provide a working clasp executable. See wscript.config and/or set the CLASP env variable."
            )
        cmd = self.clasp_command_line(
            other_clasp,
            features=["ignore-extensions"],
            resource_dir=os.path.join(
                self.bld.path.abspath(), out, self.bld.variant_obj.variant_dir()
            ),
            forms=[
                "(setq *features* (cons :cclasp *features*))",
                '(load "sys:kernel;clasp-builder.lsp")',
...
class compile_module(clasp_task):
    def run(self):
        ...
        log.debug(
            "In compile_module %s --image %s, %s -> %s",
            executable,
            image_file,
            source_file,
            fasl_file,
        ...
        cmd = self.clasp_command_line(
            ...
            features=["ignore-extensions"],
            forms=[
                '(format t "startup-linkage -> ~s~%" (multiple-value-list (core:startup-linkage)))',
                '(compile-file #P"%s" :output-file #P"%s" :output-type %s)'
                % (source_file, fasl_file, output_type),
                "(core:quit)",
            ],
        )
        return self.exec_command(cmd)
...
class link_bitcode(clasp_task):
    ...
    def run(self):
        ...
        cmd = (
            ""
            + self.env.LLVM_AR_BINARY
            + " ru %s %s" % (self.outputs[0], all_inputs.getvalue())
        )
    ...
    def scraper_command_line(self, extraCommands=[], scraperArgs=[]):
        ...
        cmd = (
            ...
            + [
                ...
                % scraper_home,
                "--eval",
                '(asdf:initialize-output-translations \'(:output-translations (t ("%s" :implementation)) :inherit-configuration))'
                % fasl_dir,
                "--eval",
                "(let ((uiop:*uninteresting-conditions* (list* 'style-warning uiop:*usual-uninteresting-conditions*)) (*compile-print* nil) (*compile-verbose* nil)) (asdf:load-system :clasp-scraper))",
            ]
            + extraCommands
            + ["--eval", "(quit)", "--end-toplevel-options"]
            + scraperArgs
        )
        return cmd
...
class precompile_scraper(scraper_task):
    weight = (
        5  # Tell waf to run this early among the equal tasks because it will take long
    )
    before = ["expand_pump_template"]

    def run(self):
        cmd = self.scraper_command_line(
            [
                "--eval",
                '(with-open-file (stream "%s" :direction :output :if-exists :supersede) (terpri stream))'
...
class generate_sif_files(scraper_task):
    ...
    def run(self):
        ...
        cmd = self.scraper_command_line(
            [
                "--eval",
                '(cscrape:generate-sif-files "%s" \'(%s))'
                % (env.BUILD_ROOT + "/", preproc_args_as_string),
            ]
        )
...
class generate_headers_from_all_sifs(scraper_task):
    ...
    def run(self):
        ...
        cmd = self.scraper_command_line(
            ...
            [
                os.path.join(
                    bld.path.abspath(), out, bld.variant_obj.variant_dir() + "/"
                ),
                env.BUILD_ROOT + "/",
            ],
        )
...
def make_pump_tasks(bld, template_dir, output_dir):
    ...
    for template_node in templates:
        ...
        output_path = os.path.join(
            "generated/", output_dir, template_name.replace(".pmp", ".h")
        )
        output_node = bld.path.find_or_declare(output_path)
        log.debug(
            "Creating expand_pump_template: %s -> %s",
            template_node.abspath(),
            output_node.abspath(),
        )
        assert output_node
        ...
        bld.add_to_group(task)
        bld.install_files(
            "${PREFIX}/lib/clasp/", [output_node], relative_trick=True, cwd=bld.path
        )
    log.info(
        "Created %s pump template tasks found in dir: %s", len(templates), template_dir
    )
...
def postprocess_all_c_tasks(self):
    ...
    log.info(
        "Creating %s parallel scraper tasks, each processing %s files, for the total %s cxx files",
        len(chunks),
        chunk_size,
        len(all_cxx_nodes),
    )
    ...
    scraper_output_nodes = [
        self.path.find_or_declare("generated/" + i) for i in scraper_generated_files
    ]

    self.create_task(
        "generate_headers_from_all_sifs", all_sif_nodes, scraper_output_nodes
    )
    # TODO FIXME this includes cl-wrappers.lisp
    install("lib/clasp/", scraper_output_nodes)  # includes

    # intrinsics
    intrinsics_bitcode_archive_node = self.path.find_or_declare(
        variant.inline_bitcode_archive_name("intrinsics")
    )
    intrinsics_bitcode_alone_node = self.path.find_or_declare(
        variant.inline_bitcode_name("intrinsics")
    )
    log.debug(
        "intrinsics_cc = %s, intrinsics_o.name = %s, intrinsics_bitcode_alone_node = %s",
        intrinsics_cc,
        intrinsics_o.name,
    ...
    # builtins
    builtins_bitcode_archive_node = self.path.find_or_declare(
        variant.inline_bitcode_archive_name("builtins")
    )
    builtins_bitcode_alone_node = self.path.find_or_declare(
        variant.inline_bitcode_name("builtins")
    )
    log.debug(
        "builtins_cc = %s, builtins_o.name = %s, builtins_bitcode_alone_node = %s",
        builtins_cc,
        builtins_o.name,
    ...
    builtins_no_debug_info_bitcode_archive_node = self.path.find_or_declare(
        variant.inline_bitcode_archive_name("builtins-no-debug-info")
    )
    builtins_no_debug_info_bitcode_alone_node = self.path.find_or_declare(
        variant.inline_bitcode_name("builtins-no-debug-info")
    )
    log.debug(
        "No debug-info builtins_cc = %s, builtins_no_debug_info_o.name = %s, builtins_no_debug_info_bitcode_alone_node = %s",
        builtins_cc,
        builtins_no_debug_info_o.name,
        builtins_no_debug_info_bitcode_alone_node,
    )
    self.create_task(
        "build_bitcode_no_debug_info",
        [builtins_cc] + scraper_output_nodes,
        builtins_no_debug_info_bitcode_alone_node,
    )
    self.create_task(
    ...
    self.create_task("link_bitcode", all_o_nodes, cxx_all_bitcode_node)
...
def increase_weight(self):
    for task in self.tasks:
        if task.__str__().find("src/asttooling/astExpose") != -1:
            task.weight = 4
        if (
            task.__str__().find("src/gctools/exposeFunctions") != -1
...
class build_clasp_extension(waflib.Task.Task):
    def run(self):
        print(
            "In build_extension self.env.enable_jupyter -> %s" % self.env.enable_jupyter
        )
